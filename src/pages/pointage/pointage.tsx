...
import {
  Search, Clock, UserCheck, LogOut, TrendingUp, CalendarDays,
  Download, Upload, ShieldAlert, CheckCircle2, XCircle, Loader2,
  CreditCard, Wifi, WifiOff, Activity, History, Settings, X,
} from "lucide-react"
import { getInitials, formatDateTime, toUpper } from "@/lib/utils"
        ...
        <div className="lg:col-span-1 space-y-6">
          ...
          <Card className="border-amber-200 bg-amber-50/30 dark:border-amber-800 dark:bg-amber-950/20">
            ...
            <CardContent className="space-y-3">
              <p className="text-xs text-muted-foreground">Saisissez le rfid manuellement :</p>
              <Input
            ...
            <CardContent>
              ...
              <ScrollArea className="h-[260px]">
                {filteredToday.length === 0 ? (
                  <p className="text-sm text-muted-foreground text-center py-8">Aucun pointage aujourd&apos;hui</p>
                ) : (
                  <div className="space-y-2">
                    {paginatedAttendance.map(a => (
                      <div key={a.id} className="flex items-center gap-3 p-2 rounded-lg border bg-card text-sm">
                        <Avatar className="h-7 w-7">
                          {a.member?.photo_url ? <AvatarImage src={a.member.photo_url} /> : null}
                          <AvatarFallback className="text-[10px]">{getInitials(a.member?.first_name ?? "", a.member?.last_name ?? "")}</AvatarFallback>
                        </Avatar>
                        <div className="flex-1 min-w-0">
                          <p className="font-medium truncate">{toUpper(`${a.member?.first_name ?? ""} ${a.member?.last_name ?? ""}`)}</p>
                        </div>
                        <div className="flex items-center gap-2 text-xs shrink-0">
                          <span className="text-success">{formatTime(a.check_in)}</span>
                          {a.check_out && <span className="text-destructive">{formatTime(a.check_out)}</span>}
                        </div>
                      </div>
                    ))}
                  </div>
